...
func CreateTempGoWork(ms *ModuleSet) (string, error) {
	...
	// Deduplicate by both absolute dir and module path from go.mod.
	seenDirs := make(map[string]bool, len(ms.Dirs()))
	seenModulePaths := make(map[string]bool, len(ms.Dirs()))
	addDir := func(dir string) {
		if seenDirs[dir] {
			return
		}
		modPath, err := modulePathFromGoMod(dir)
		if err == nil && modPath != "" {
			if seenModulePaths[modPath] {
				return
			}
			seenModulePaths[modPath] = true
		}
		seenDirs[dir] = true
		buf.WriteString("\t" + strconv.Quote(dir) + "\n")
	}

	// Add top-level module dirs first.
	for _, m := range ms.Dirs() {
		addDir(m.Dir)
	}

	// Walk all module directories to include nested submodules.
	for _, m := range ms.Dirs() {
